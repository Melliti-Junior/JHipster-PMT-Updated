...
@Component({
    selector: 'jhi-status-custom',
    templateUrl: './status-custom.component.html'
})
export class StatusCustomComponent implements OnInit, OnDestroy {
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/statuscustoms', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: StatusCustom) {
        return item.id;
    }
    registerChangeInStatuses() {
        this.eventSubscriber = this.eventManager.subscribe('statuscustomsListModification', (response) => this.loadAll());
    }

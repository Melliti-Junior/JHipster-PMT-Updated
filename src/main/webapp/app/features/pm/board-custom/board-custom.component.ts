...
@Component({
    ...
    templateUrl: './board-custom.component.html'
})
export class BoardCustomComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.boardcustomService.search({
                page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/boardcustoms'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/boardcustoms', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: BoardCustom) {
        return item.id;
    }
